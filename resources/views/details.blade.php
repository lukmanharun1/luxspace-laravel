    ...
    <section class="bg-gray-100 py-8 px-4">
        <div class="container mx-auto">
            <ul class="breadcrumb">
                ...
                <li>
                    @php
                        $categories = explode('_', $details->category);
                        $category = ucfirst($categories[0]) . ' ' . ucfirst($categories[1]);
                    @endphp
                    <a href="{{ url('category-room/' . $details->category) }}">{{ $category }}</a>
                </li>
                <li>
    ...
    <section class="container mx-auto">
        <div class="flex flex-wrap my-4 md:my-12">
            <div class="w-full md:hidden px-4">
                <h2 class="text-3xl lg:text-5xl font-semibold mb-3">{{ $details->name_product }}</h2>
                <span class="text-xl">IDR {{ number_format($details->price, 0, ',', '.') }}</span>
            </div>
            <div class="flex-1">
                <div class="slider gap-6">
                    <div class="thumbnail gap-2 md:ml-3 mx-2 sm:mx-0">
                        <!-- START: slideshow thumbnail items -->
                        @foreach ($images as $i => $image)
                            <div class="mx-1">
                                <div class="item @if ($i == 0) selected @endif"
                                    data-img="{{ asset('images/upload_images/' . $image) }}">
                                    <img src="{{ asset('images/upload_images/' . $image) }}"
                                        alt="{{ $details->name_product }}" class="object-cover w-full h-full rounded-lg" />
                                </div>
                            </div>
                        @endforeach
                        <!-- END: slideshow thumbnail items -->
                    </div>
                    <div class="preview">
                        <div class="item rounded-lg h-full overflow-hidden">
                            <img src="{{ asset('images/upload_images/' . $details->image1) }}"
            ...
            <div class="flex-1 px-4 md:p-6">
                ...
                <a href="/add-to-cart/{{ $details->id }}"
                    class="transition-all duration-200 bg-pink-400 text-black hover:bg-black hover:text-pink-400 rounded-full px-8 py-3 mt-4 inline-flex md:w-auto w-full justify-center"><svg
                        class="fill-current mr-3" width="26" height="25" viewBox="0 0 26 25" fill="none"
                        ...
                        <a href="{{ url('details/' . $room->id) }}">
                            <div class="rounded-xl p-4 relative bg-white">
                                <div class="rounded-xl overflow-hidden card-shadow w-full h-48">
                                    <img src="{{ asset('images/upload_images/' . $room->image1) }}" alt="chair 1"
